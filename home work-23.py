...
class Square(Shape):
    ...
    def show_square(self,visota,shirina):
...
class Rectangle(Shape):
    def __init__(self,visota,shirina):
        super().__init__(visota,shirina)
    def show_rectangle(self,visota,shirina):
        for i in range(visota):
            for j in range(shirina):
                print('*', end=' ')
            print("*")

    def save_sqaure(self):
        fiqura = self.show_rectangle
        from pickle import dump, load
        doc_file = r'C:\Users\FX505DT\Desktop\file.bin\file.doc'
...
square.show_square(10,10)
rectangle.show_rectangle(5,20)
